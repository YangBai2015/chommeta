...
plot_btree <- function(tree, output.file="Rplots.pdf", cex.labels=1, label.offset=5.3,
                       ...
                       barplots.height=1, barplots.width, barplots.col) {

	if (grepl(".svg", output.file)) {

        svg(file=output.file, height=7, width=9)

    } else {

        pdf(file=output.file, height=7, width=9)
	...
	if(type=="fan") {
        ...
        for (j in 1:dim(barplots.height)[1]) {
            ...
            w <- barplots.width
            offset <- .025
            x1 <- x2 <- max(xx.labels) + offset * i + label.offset
            x3 <- x4 <- max(xx.labels) + offset * i + label.offset + as.numeric(barplots.height[j, i])
            y1 <- y4 <- yy.tips[j] - w
            y2 <- y3 <- yy.tips[j] + w
    ...
    for (i in 1:length(xx.tips)) {
        ...
        points(x=xx.tips[i], y=yy.tips[i],
               ...
               cex=tips.cex[i], col=tips.col[i], lwd=.0)
